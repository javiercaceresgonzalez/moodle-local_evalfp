...
/**
 * Form for creating or editing a course CE.
 *
 * CE records always belong to one course RA and keep their code unique within
 * that parent RA.
 *
 ...
 */
class ce extends \moodleform {
    /**
     * Defines the CE code and description fields.
     *
     * @return void
     */
    public function definition() {
        ...
        $mform->addElement('hidden', 'courseceid');
        $mform->setType('courseceid', PARAM_INT);

        if (!empty($this->_customdata['ra'])) {
            $ra = $this->_customdata['ra'];
            $displayra = \local_evalfp_format_ra_code($ra);
            $rahtml = \html_writer::span($displayra, 'badge badge-primary mr-2') . format_string($ra->description);
            $mform->addElement('static', 'ra_context', get_string('form_ra_description', 'local_evalfp'), $rahtml);
        }
        ...
        $mform->addElement(
            'textarea',
            'description',
            get_string('form_ce_description', 'local_evalfp'),
            ['rows' => 4, 'cols' => 64]
        );
    ...
    public function validation($data, $files) {
        ...
        $params = ['courseraid' => $data['courseraid'], 'code' => trim($data['code'] ?? '')];
        $exists = $DB->get_record('local_evalfp_course_ce', $params, 'id', IGNORE_MISSING);
        if ($exists && (empty($data['courseceid']) || (int)$exists->id !== (int)$data['courseceid'])) {
